...
// ----------------- SISTEMA DE RARIDADE ------------------
RarityBonus getRarityBonusByName(const std::string& itemName) {
	if (itemName.find("[M]") != std::string::npos)
		return RarityBonus(3, 3); // MÍTICO
	if (itemName.find("[L]") != std::string::npos)
		return RarityBonus(2, 3); // LENDÁRIO
	if (itemName.find("[E]") != std::string::npos)
		return RarityBonus(2, 2); // ÉPICO
	if (itemName.find("[R]") != std::string::npos)
		return RarityBonus(1, 2); // RARO
	if (itemName.find("[N]") != std::string::npos)
		return RarityBonus(1, 1); // NORMAL
	return RarityBonus(); // Sem bônus
}
// --------------------------------------------------------
...
int32_t MoveEventScript::luaCallFunction(lua_State* L)
{
	...
	if(event->getEventType() == MOVE_EVENT_EQUIP || event->getEventType() == MOVE_EVENT_DE_EQUIP)
	{
		...
		if(event->getEventType() != MOVE_EVENT_EQUIP)
			lua_pushboolean(L, MoveEvent::DeEquipItem(event, player, item, slot, boolean, false));
		else
			lua_pushboolean(L, MoveEvent::EquipItem(event, player, item, slot, boolean, false));
...
bool MoveEvent::EquipItem(MoveEvent* moveEvent, Player* player, Item* item, slots_t slot, bool isCheck, bool isLogin) {
	...
	if (!isLogin) {
		for (CreatureEvent* it : player->getCreatureEvents(CREATURE_EVENT_EQUIP_ITEM)) {
			it->executeEquipItemEvent(player, item, slot);
		}
	}
	...
	// Remove raridade anterior (se existir)
RarityBonus& previousBonus = player->raritySlotBonus[slot];
if (previousBonus.magicLevel > 0) {
	player->setVarStats(STAT_MAGICLEVEL, -previousBonus.magicLevel);
	player->sendStats();
}
if (previousBonus.skillFist > 0) {
	player->setVarSkill(SKILL_FIST, -previousBonus.skillFist);
	player->sendSkills();
}

// Aplica nova raridade
RarityBonus newBonus = getRarityBonusByName(item->getName());
player->raritySlotBonus[slot] = newBonus;

if (newBonus.magicLevel > 0) {
	player->setVarStats(STAT_MAGICLEVEL, newBonus.magicLevel);
	player->sendStats();
}
if (newBonus.skillFist > 0) {
	player->setVarSkill(SKILL_FIST, newBonus.skillFist);
	player->sendSkills();
}
	return true;
}

bool MoveEvent::DeEquipItem(MoveEvent* moveEvent, Player* player, Item* item, slots_t slot, bool isRemoval, bool isLogout) {
	...
	if (!isLogout) {
		for (auto ev : player->getCreatureEvents(CREATURE_EVENT_UNEQUIP_ITEM)) {
			ev->executeEquipItemEvent(player, item, slot);
		}
	}
	...
	RarityBonus& bonus = player->raritySlotBonus[slot];
if (bonus.magicLevel > 0) {
	player->setVarStats(STAT_MAGICLEVEL, -bonus.magicLevel);
	player->sendStats();
}
if (bonus.skillFist > 0) {
	player->setVarSkill(SKILL_FIST, -bonus.skillFist);
	player->sendSkills();
}

// Limpa o slot
player->raritySlotBonus.erase(slot);
	return true;
}
...
bool MoveEvent::fireEquip(Player* player, Item* item, slots_t slot, bool boolean)
{
	...
	if (equipFunction == MoveEvent::EquipItem) {
		return equipFunction(this, player, item, slot, boolean, false);
	}
	else {
		return equipFunction(this, player, item, slot, boolean, false);
	}
