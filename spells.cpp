...
bool Spell::configureSpell(xmlNodePtr p)
{
	...
	if (readXMLString(p, "name", strValue))
	{
		...
		const char* reservedList[] =
		{
			...
			"cursecondition", "suiton", "fuuton", "doton", "raiton", "mokuton", "shoton", "suna", "enton", "hyoton", 
			"suitoncondition", "fuutoncondition", "dotoncondition", "raitoncondition", "mokutoncondition", "shotoncondition", 
			"sunacondition", "entoncondition", "hyotoncondition", "yoton", "ranton", "bakuton", "shakuton", "futton", "jinton", "satetsu", "taijutsu", 
			"yin", "yang", "kenjutsu", "yotoncondition", "rantoncondition", "bakutoncondition", "shakutoncondition",
			"futtoncondition", "jintoncondition", "satetsucondition", "taijutsucondition", "yincondition", "yangcondition", "kenjutsucondition"
		};
	...
	if (readXMLString(p, "desc", strValue))
		desc = strValue;
	
	if (readXMLString(p, "cooldownID", strValue))
		cooldownID = strValue;
	
	if (readXMLInteger(p, "cooldownValue", intValue))
		cooldownValue = intValue;
...
bool Spell::checkSpell(Player* player) const
{
	...
	std::string stringCD;
    if (player->getStorage(getCooldownID(), stringCD)) {
		uint64_t CD;
		std::stringstream cd;
	    cd << stringCD;
		cd >> CD;
...
void Spell::postSpell(Player* player, uint32_t manaCost, uint32_t soulCost) const
{
	...
	std::stringstream storageValue;
	storageValue << OTSYS_TIME() + getCooldownValue();
	player->setStorage(getCooldownID(), storageValue.str());
}
...
bool InstantSpell::Levitate(const InstantSpell*, Creature* creature, const std::string& param)
{
    ...
    if (tmpParam == "up") {
		std::vector<uint16_t> possibleIDs = {13676, 13686, 13688, 12419, 876, 875, 4472, 459, 4468, 4469, 19475, 19477, 19502, 4470, 4471, 4473, 4474, 4475, 4476, 4477, 4478, 4479, 4480, 4481, 4482, 4483, 4484, 4485, 4486, 4487, 4488, 4489, 4490, 4491, 4492, 4493, 4494, 4495, 4496, 4497, 4498, 4499, 4500, 4501, 4502, 4503, 4504, 4505, 4506, 4507, 4508, 4509, 4510, 4511, 4512, 4513, 878, 903, 904, 906, 907, 908, 909, 910, 911, 912, 913, 11249, 11250, 11251, 11252, 11253, 11254, 11255, 11256, 11257, 11258, 11259, 11260, 11261, 11262, 11263, 11264, 11265, 11266, 11267, 11268, 11269, 11270, 11271, 11272, 11273};
        
		const Position& playerPos = player->getPosition();
        Tile* tileInFront = nullptr;

        switch (player->getDirection()) {
            case NORTH:
                tileInFront = g_game.getTile(playerPos.x, playerPos.y - 1, playerPos.z);
                break;
            case EAST:
                tileInFront = g_game.getTile(playerPos.x + 1, playerPos.y, playerPos.z);
                break;
            case SOUTH:
                tileInFront = g_game.getTile(playerPos.x, playerPos.y + 1, playerPos.z);
                break;
        ...
        bool validItemInFront = false;
        uint16_t itemCount = tileInFront->getThingCount();
        if (itemCount > 0) {
            for (int i = itemCount - 1; i >= 0; --i) {
                if (Item* item = tileInFront->__getThing(i)->getItem()) {
                    for (uint16_t id : possibleIDs) {
                        if (item->getID() == id) {
                            validItemInFront = true;
                            break;
                        }
                    }
                }
		...
		if (!validItemInFront) {
			...
			if (position.z != floor) {
				...
				if (!tmpTile || (!tmpTile->ground && !tmpTile->hasProperty(IMMOVABLEBLOCKSOLID))) {
					...
					tmpTile = g_game.getTile(destination);
					if (tmpTile && tmpTile->ground && !tmpTile->hasProperty(IMMOVABLEBLOCKSOLID) &&
						tile && tile->hasFlag(TILESTATE_HOUSE) == tmpTile->hasFlag(TILESTATE_HOUSE) &&
			...
			if (ret == RET_NOERROR) {
				g_game.addMagicEffect(player->getPosition(), 735, player->isGhost());
				return true;
			} else {
				player->sendCancelMessage(ret);
		...
		std::vector<uint16_t> itemIDs = {9536, 9535, 4631};

		bool validItemBelow = false;
		Tile* playerTile = g_game.getTile(player->getPosition());
		if (playerTile) {
			uint16_t itemCount = playerTile->getThingCount();
			if (itemCount > 0) {
				for (int i = itemCount - 1; i >= 0; --i) {
					if (Item* item = playerTile->__getThing(i)->getItem()) {
						if (std::find(itemIDs.begin(), itemIDs.end(), item->getID()) != itemIDs.end()) {
							validItemBelow = true;
							break;
						}
					}
