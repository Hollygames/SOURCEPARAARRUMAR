...
struct RarityBonus {
	...
	RarityBonus(int ml = 0, int sf = 0)
		: magicLevel(ml), skillFist(sf) {}
};
...
class Player : public Creature, public Cylinder
{
	public:
		...
		std::map<slots_t, RarityBonus> raritySlotBonus;
		...
		//defense para players
		void setDefense(int32_t value);
		...
		void sendCreatureJump(uint32_t creatureId, uint32_t height, uint32_t duration, bool straight)
		{
			if (client) {
				client->sendCreatureJump(creatureId, height, duration, straight);
			}
		}
	...
	protected:
		...
		void updateBaseSpeed()
		{
			if(!hasFlag(PlayerFlag_SetMaxSpeed))
				baseSpeed = vocation->getBaseSpeed() + (1 * (level - 1)); // speed por nivel
			else
				baseSpeed = SPEED_MAX;
		}
		...
		int32_t getAdditionalDefense() const;
