		...
		if(Player* player = creature->getPlayer())
		{
			...
			switch(formulaType)
			{
				...
				case FORMULA_SKILL:
				{
					...
					if (target->isImmuneWeaponDamage()) {
		            g_game.addAnimatedText(creature->getPosition(), COLOR_WHITE, "MISS");
					max = 0;
					}
                    min = (int32_t)max/100*80;
					return true;
				}
...
CombatType_t Combat::ConditionToDamageType(ConditionType_t type)
{
	switch(type)
	{
		...
		case CONDITION_SUITON:
			return COMBAT_SUITONDAMAGE;
			
		case CONDITION_FUUTON:
			return COMBAT_FUUTONDAMAGE;
			
		case CONDITION_DOTON:
			return COMBAT_DOTONDAMAGE;
			
		case CONDITION_RAITON:
			return COMBAT_RAITONDAMAGE;
			
		case CONDITION_MOKUTON:
			return COMBAT_MOKUTONDAMAGE;
			
		case CONDITION_SHOTON:
			return COMBAT_SHOTONDAMAGE;
			
		case CONDITION_SUNA:
			return COMBAT_SUNADAMAGE;
			
		case CONDITION_ENTON:
			return COMBAT_ENTONDAMAGE;
			
		case CONDITION_HYOTON:
			return COMBAT_HYOTONDAMAGE;
		
		case CONDITION_YOTON:
			return COMBAT_YOTONDAMAGE;
		
		case CONDITION_RANTON:
			return COMBAT_RANTONDAMAGE;

		case CONDITION_BAKUTON:
			return COMBAT_BAKUTONDAMAGE;
		
		case CONDITION_SHAKUTON:
			return COMBAT_SHAKUTONDAMAGE;

		case CONDITION_FUTTON:
			return COMBAT_FUTTONDAMAGE;

		case CONDITION_JINTON:
			return COMBAT_JINTONDAMAGE;

		case CONDITION_SATETSU:
			return COMBAT_SATETSUDAMAGE;

		case CONDITION_TAIJUTSU:
			return COMBAT_TAIJUTSUDAMAGE;

		case CONDITION_YIN:
			return COMBAT_YINDAMAGE;
		
		case CONDITION_YANG:
			return COMBAT_YANGDAMAGE;
		
		case CONDITION_KENJUTSU:
			return COMBAT_KENJUTSUDAMAGE;
...
ConditionType_t Combat::DamageToConditionType(CombatType_t type)
{
	switch(type)
	{
		...
		case COMBAT_SUITONDAMAGE:
			return CONDITION_SUITON;
			
		case COMBAT_FUUTONDAMAGE:
			return CONDITION_FUUTON;
			
		case COMBAT_DOTONDAMAGE:
			return CONDITION_DOTON;
			
		case COMBAT_RAITONDAMAGE:
			return CONDITION_RAITON;
			
		case COMBAT_MOKUTONDAMAGE:
			return CONDITION_MOKUTON;
			
		case COMBAT_SHOTONDAMAGE:
			return CONDITION_SHOTON;
			
		case COMBAT_SUNADAMAGE:
			return CONDITION_SUNA;
			
		case COMBAT_ENTONDAMAGE:
			return CONDITION_ENTON;
			
		case COMBAT_HYOTONDAMAGE:
			return CONDITION_HYOTON;
		
		case COMBAT_YOTONDAMAGE:
			return CONDITION_YOTON;
		
		case COMBAT_RANTONDAMAGE:
			return CONDITION_RANTON;

		case COMBAT_BAKUTONDAMAGE:
			return CONDITION_BAKUTON;
		
		case COMBAT_SHAKUTONDAMAGE:
			return CONDITION_SHAKUTON;

		case COMBAT_FUTTONDAMAGE:
			return CONDITION_FUTTON;

		case COMBAT_JINTONDAMAGE:
			return CONDITION_JINTON;

		case COMBAT_SATETSUDAMAGE:
			return CONDITION_SATETSU;

		case COMBAT_TAIJUTSUDAMAGE:
			return CONDITION_TAIJUTSU;

		case COMBAT_YINDAMAGE:
			return CONDITION_YIN;
			
		case COMBAT_YANGDAMAGE:
			return CONDITION_YANG;
			
		case COMBAT_KENJUTSUDAMAGE:
			return CONDITION_KENJUTSU;
...
ReturnValue Combat::canDoCombat(const Creature* attacker, const Creature* target, bool isAggressive)
{
    ...
    if(attacker->isBattleLock()) // BattleLock
		return RET_BATTLELOCK;
	...
	if(const Player* targetPlayer = target->getPlayer())
	{
		...
		if (targetPlayer->isBattleLock())
			return RET_YOUMAYNOTATTACKTHISPLAYER;  // BattleLock
...
ReturnValue Combat::canTargetCreature(const Player* player, const Creature* target)
{
	...
	if(player->isBattleLock()) // BattleLock
		return RET_BATTLELOCK;
	
	if(target->isBattleLock())  // BattleLock
		return RET_YOUMAYNOTATTACKTHISPLAYER;
    ...
    if(target->getPlayer() && player->getSecureMode() == SECUREMODE_ON) // optional pvp
		return RET_TURNSECUREMODETOATTACKUNMARKEDPLAYERS;
    ...
    const Creature* creature = player->getCreature(); // Saga System
    if (target->getMonster() && creature) {
		uint8_t error = const_cast<Monster*>(target->getMonster())->isPlayerSagaBlock(creature);
		if (error == 2)
		return RET_MISSAOERROR;
		else if (error == 1) 
		return RET_SAGAERROR; 
	}
...
bool Combat::CombatHealthFunc(Creature* caster, Creature* target, const CombatParams& params, void* data)
{
	...
	if (params.combatType != COMBAT_HEALING) {
		// --- AUMENTO DE DANO (CASTER) ---
		int32_t totalBonusPercent = 0;

		if (caster && caster->getPlayer()) {
			std::string value;

			// Storage 1 - bônus de dano
			caster->getStorage("76442564", value);
			totalBonusPercent += atoi(value.c_str());

			// Storage 2 - bônus extra
			caster->getStorage("15552", value);
			totalBonusPercent += atoi(value.c_str());

			if (totalBonusPercent > 0) {
				change = (int32_t)std::ceil(change + change * totalBonusPercent / 100.0);
			}

			// --- REDUÇÃO DE DANO (DEBUFF NO CASTER) ---
			int32_t casterReductionPercent = 0;
			caster->getStorage("15555", value);
			casterReductionPercent = std::min(100, atoi(value.c_str()));

			if (casterReductionPercent > 0) {
				change = (int32_t)std::ceil(change - change * casterReductionPercent / 100.0);

				// Mensagem de debuff como texto animado no chão
				if (change <= 0) {
					g_game.addAnimatedText(caster->getPosition(), COLOR_WHITE, "Debuff!");
				}
			}
		}

		// --- AUMENTO/REDUÇÃO DE DANO POR ELEMENTO ---
		if (caster && target && caster->getPlayer() && target->getPlayer()) {
			std::string casterElement, targetElement;

			caster->getStorage("15556", casterElement);
			target->getStorage("15556", targetElement);

			std::transform(casterElement.begin(), casterElement.end(), casterElement.begin(), ::tolower);
			std::transform(targetElement.begin(), targetElement.end(), targetElement.begin(), ::tolower);
			...
			if (change < 0) {
				...
				if (casterElement == "fogo") {
					if (targetElement == "vento") {
						change = (int32_t)std::ceil(change + change * 10 / 100.0);
						isAdvantage = true;
						modified = true;
					} else if (targetElement == "agua") {
						change = (int32_t)std::ceil(change - change * 5 / 100.0);
						isDisadvantage = true;
						modified = true;
					}
				} else if (casterElement == "vento") {
					if (targetElement == "terra") {
						change = (int32_t)std::ceil(change + change * 10 / 100.0);
						isAdvantage = true;
						modified = true;
					} else if (targetElement == "fogo") {
						change = (int32_t)std::ceil(change - change * 5 / 100.0);
						isDisadvantage = true;
						modified = true;
					}
				} else if (casterElement == "terra") {
					if (targetElement == "raio") {
						change = (int32_t)std::ceil(change + change * 10 / 100.0);
						isAdvantage = true;
						modified = true;
					} else if (targetElement == "vento") {
						change = (int32_t)std::ceil(change - change * 5 / 100.0);
						isDisadvantage = true;
						modified = true;
					}
				} else if (casterElement == "raio") {
					if (targetElement == "agua") {
						change = (int32_t)std::ceil(change + change * 10 / 100.0);
						isAdvantage = true;
						modified = true;
					} else if (targetElement == "terra") {
						change = (int32_t)std::ceil(change - change * 5 / 100.0);
						isDisadvantage = true;
						modified = true;
					}
				} else if (casterElement == "agua") {
					if (targetElement == "fogo") {
						change = (int32_t)std::ceil(change + change * 10 / 100.0);
						isAdvantage = true;
						modified = true;
					} else if (targetElement == "raio") {
						change = (int32_t)std::ceil(change - change * 5 / 100.0);
						isDisadvantage = true;
						modified = true;
					}
				...
				if (modified) {
					...
					if (isAdvantage && chance < 20) {
						g_game.addAnimatedText(target->getPosition(), COLOR_ORANGE, "Elemento!");
					} else if (isDisadvantage && chance < 20) {
						g_game.addAnimatedText(target->getPosition(), COLOR_TEAL, "Absorb!");
					}
				}
			}
		}

		// --- REDUÇÃO DE DANO (DEFESA DO TARGET) ---
		int32_t totalReductionPercent = 0;

		if (target && target->getPlayer() && change < 0) {
			std::string value;

			// Storage 1
			target->getStorage("15553", value);
			totalReductionPercent += atoi(value.c_str());

			// Storage 2
			target->getStorage("15554", value);
			totalReductionPercent += atoi(value.c_str());

			// Limita a 100%
			if (totalReductionPercent > 0) {
				totalReductionPercent = std::min(100, totalReductionPercent);
				change = (int32_t)std::ceil(change - change * totalReductionPercent / 100.0);
			}
		}
	}
	...
	if (caster && caster->getPlayer() && target && target->getPlayer() && target->getSkull() != SKULL_BLACK) {
...
void MagicField::onStepInField(Creature* creature)
{
	...
	if (it.condition) {
		...
		if (ownerId && !getTile()->hasFlag(TILESTATE_HARDCOREZONE)) {
			...
			if (!harmfulField || (OTSYS_TIME() - createTime <= (uint32_t)g_config.getNumber(
